        ...
        <div className="bg-white rounded-2xl shadow-xl border border-amber-100 overflow-hidden mb-6">
          <div className="grid grid-cols-1 lg:grid-cols-6">
            ...
            <div className="lg:col-span-2 p-6 border-r border-amber-100">
              {/* Dropdown Navigation - CHANGED TO GREEN */}
              <div className="relative mb-6">
                <button
                  onClick={() => setShowDropdown(!showDropdown)}
                  className="w-full flex items-center justify-between px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-colors text-sm font-medium"
                >
                ...
                {showDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-1 rounded-xl shadow-lg z-10">
                    <button
                      ...
                      className="w-full flex items-center space-x-2 px-4 py-3 text-left text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 rounded-t-xl transition-colors text-sm"
                    >
                      <Upload size={16} />
                    ...
                    <button
                      ...
                      className="w-full flex items-center space-x-2 px-4 py-3 text-left text-white bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 rounded-b-xl border-t border-green-400 transition-colors text-sm"
                    >
                      <Camera size={16} />
              ...
              {detectionResult && detectionResult.rawAccuracy >= 80 && (
                <div className="text-center p-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg border border-green-300 mb-4">
                  <h3 className="text-xl font-bold text-white mb-1">
                    {detectionResult.mudraName}
                  </h3>
                  <div className="inline-flex items-center space-x-1 bg-white/20 px-3 py-1 rounded-full border border-white/30 backdrop-blur-sm">
              ...
              {detectionResult ? (
                detectionResult.rawAccuracy >= 80 ? (
                  ...
                  <div className="h-full space-y-6">
                    <div className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-xl p-6 border border-rose-200 text-center">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center space-x-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-lg hover:from-amber-600 hover:to-orange-600 transition-all font-semibold text-lg mx-auto"
                      >
                        <Library size={20} />
                ...
                <div className="h-full space-y-8">
                  ...
                  <div className="bg-white rounded-xl p-5 border border-amber-200 shadow-sm">
                    <h3 className="font-semibold text-amber-900 text-2xl mb-6 flex items-center">
                      <Target size={24} className="mr-3 text-amber-600" />
                      How to Use:
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {[
                        { step: "1", title: "Select Input Method", desc: "Choose between uploading an image or using camera", color: "from-green-500 to-emerald-600" },
                        { step: "2", title: "Provide Image", desc: "Upload file or capture photo with camera", color: "from-amber-500 to-orange-500" },
                        { step: "3", title: "Analyze Mudra", desc: "Click analyze to identify the hand gesture", color: "from-amber-500 to-rose-500" },
                        { step: "4", title: "Learn & Explore", desc: "Discover meaning, symbolism, and techniques", color: "from-amber-400 to-orange-500" }
                      ].map((item, index) => (
                        <div key={index} className="flex items-start space-x-4 p-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg hover:from-amber-100 hover:to-orange-100 transition-all duration-300 border border-amber-200">
                          <div className={`w-8 h-8 bg-gradient-to-r ${item.color} rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0`}>
                            {item.step}
                          </div>
                          <div>
                    ...
                    <div className="mt-6 flex items-center space-x-3 p-4 bg-gradient-to-r from-amber-100 to-yellow-100 rounded-lg border border-amber-200">
...
const DetailItem = ({ title, content, icon, gradient = "from-amber-50 to-orange-50" }) => {
  return (
    <div className={`bg-gradient-to-br ${gradient} rounded-xl p-5 border border-amber-200 shadow-sm`}>
      <div className="flex items-start space-x-4">
        <div className="w-12 h-12 bg-white rounded-lg flex items-center justify-center border border-amber-200 flex-shrink-0 shadow-sm">
          {icon}
        </div>
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-amber-900 text-xl mb-3">{title}</h4>
          <p className="text-amber-700 text-base leading-relaxed">{content}</p>
        </div>
      </div>
    </div>
  );
